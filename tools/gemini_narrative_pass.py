...
def main() -> int:
    ...
    system = (
        ...
        "  walk through each distinct moment shown — roughly one clause or sentence per kept\n"
        "  panel-moment. A group with several panels gets a fuller, multi-beat narration; a\n"
        "  single-panel group gets one tight line. Do NOT pad, but do NOT skip panels — every\n"
        "  kept panel earns its moment of narration, so the spoken line naturally covers the\n"
        "  time those panels are on screen. Write like a top manhwa-recap channel: NAME the\n"

...
Input: manifest.stitch.json (from chunk_stitch*.py)
Output: manifest.panels.json

For each stitched chunk image, asks Gemini to return panel bounding boxes
in normalized coords: [ymin, xmin, ymax, xmax].

Fixes:
- Uses response_mime_type="application/json" + response_schema so resp.parsed works
- Robust JSON extraction fallback
- Repair pass if output is truncated / invalid
- Resume mode + retries
- Optional NMS + min area filtering

Requires:
  pip install -U google-genai pillow numpy
Auth:
...
def call_panels(
    ...
    # Keep prompt SHORT and force compact JSON.
    system = (
        ...
        "- Each bbox must tightly cover the FULL panel artwork (characters/background).\n"
        "- Panels are usually stacked top-to-bottom; return them in reading order.\n"
    ...
    user = {
        ...
        "rules": [
            ...
            "Exclude speech bubbles unless they are inside a panel (still bbox the panel).",
            "Avoid thin strips: every bbox should cover a meaningful frame, not a band.",
            "Do not output duplicates.",
            f"Return at most {max_panels} panels.",
        ],
        "quality_checks": [
            "If unsure, return fewer boxes rather than many thin slices.",
            "Boxes should have visible artwork content and clear top/bottom borders."
        ]
    }
...
def main() -> int:
    ...
    ap.add_argument("--min-h-frac", type=float, default=0.035)   # drop thin bands
    ap.add_argument("--min-w-frac", type=float, default=0.40)    # panels should be wide in webtoons
    ap.add_argument("--drop-fullwidth-bands", action="store_true")    
    ...
    for ch in chunks:
        ...
        for attempt in range(args.retries + 1):
            ...
            # Repair pass (no image needed; ask to re-emit JSON)
            repair_schema = schema
            repair_prompt = {
                "instruction": "Re-output ONLY valid JSON matching the schema. No extra text.",
                "last_output": (raw_text or "")[:5000],
            }
            resp = client.models.generate_content(
                model=args.model,
        ...
        for p in (result_obj.get("panels") or []):
            if isinstance(p, dict) and "bbox" in p:
                panels.append(p["bbox"])

        boxes = sanitize_boxes(panels)

        # drop thin bands / tiny junk BEFORE area + NMS
        ...
        for y0, x0, y1, x1 in boxes:
            ...
            if h < min_h:  # kills thin bands
                continue
            if w < min_w:  # kills narrow regions
                continue

    ...
    for line in raw.splitlines():
        ...
        for pat in cfg.ui_noise_patterns:
            if re.search(pat, s, flags=re.IGNORECASE):
                # drop common UI lines; if this ever removes dialog, soften patterns
                noisy = True
                break
        if noisy:
            continue
...
def merge_words_into_text_blocks(
    ...
    # compute bounding rect per line
    ...
    # merge near-adjacent lines into paragraphs if they overlap horizontally a lot
    blocks_px.sort(key=lambda r: r[1])  # by top y
    ...
    for r in blocks_px:
        if not merged:
            ...
            continue
        p = merged[-1]
        # if close vertically, and horizontal overlap >= 0.35, merge
        gap = r[1] - p[3]
        if gap <= merge_y_px:
    ...
    # normalize and keep biggest blocks
...
def make_targets(
    ...
    for i, o in enumerate(objects, 1):
        # object bbox is already normalized here
        bb = o.get("bbox")
...
_STRONG_VISUAL_TERMS = {
    # protect against “phone screen text” false positives
...
def classify_text_only(ocr_clean: str, text_cov: float, labels: List[str], objects: List[str], cfg: VisionConfig) -> bool:
    """
    Intended meaning: “mostly speech bubbles / text panel”, not “phone UI text”.
    """
    ...
    # if text coverage is genuinely high and visuals are weak -> likely text-only
    if text_cov >= max(0.20, cfg.text_only_min_coverage):
        return True

    # allow a softer threshold only when “Text/font” appears early and we don't have strong visuals
    ...
    for p in paths:
        ...
        try:
            ...
            for o in objects_raw:
                bb = rect_from_norm_vertices(getattr(o, "bounding_poly", None).normalized_vertices if getattr(o, "bounding_poly", None) else None)
                objects.append(
                    {
                        "name": o.name,
                        "score": float(o.score),
            ...
            for fa in list(resp.face_annotations or [])[: cfg.max_faces]:
                ...
                r_n = rect_px_to_norm(r_px, w, h)
                faces.append({"bbox": [float(r_n[0]), float(r_n[1]), float(r_n[2]), float(r_n[3])], "confidence": float(getattr(fa, "detection_confidence", 0.0) or 0.0)})
            ...
            targets = make_targets(w, h, text_blocks=text_blocks, objects=objects, faces=faces)
            ...
            items.append(
                {
                    ...
                    # keep your original “vision payload” (compact)
                    "vision": {
                        "text": full_text,
                        "labels": labels,
                        "objects": objects,
                        "faces": faces,
                        "text_blocks": [[round(x, 4) for x in b] for b in text_blocks],  # redundant, but handy
                    },
                }
            )

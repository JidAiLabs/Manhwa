...
def choose_segments_by_score(
    ...
    Pipeline:
      1) Score each segment (edge density) with a minimum height gate.
      2) Build candidate list:
           score >= abs_thr AND score >= max_score * rel_thr
      3) If no candidates, keep best-score segment only.
      4) Rank candidates by score, slight height tie-break.
      5) Dominance suppression:
           Always keep best.
           Keep additional segments only if:
             - segment is sufficiently tall, OR
             - score is sufficiently close to best AND not tiny.
           If the best already covers most of the page, apply stricter criteria.
    """
    if not segs:
        return [], []
    ...
    # --- candidate gate (non-blank-ish) ---
    cand_idx = [
        i for i, (s, sc) in enumerate(zip(segs, scores))
        if s.h >= min_h_px and sc >= abs_thr and sc >= (max_score * rel_thr if max_score > 0 else abs_thr)
    ]

    # If everything looks blank, keep best score segment only
    if not cand_idx:
        best_i = int(max(range(len(segs)), key=lambda i: scores[i]))
        return [segs[best_i]], [scores[best_i]]

    # --- rank (score first, slight height tie-break) ---
    ...
    # --- dominance suppression / redundancy control ---
    best_i = cand_idx[0]
    best_sc = scores[best_i]
    best_h_ratio = segs[best_i].h / max(1, H)

    # Tunables (good starting points)
    SCORE_KEEP_RATIO = 0.80     # keep if >= 80% of best score
    MIN_H_KEEP_RATIO = 0.22     # keep if >= 22% of full height (even if score lower)
    MIN_H_FOR_SCORE = 0.14      # if relying on score closeness, don't allow very tiny strips

    # If best already covers most of the page, be stricter to avoid tiny extra strips
    BIG_BEST_H = 0.62
    STRICT_SCORE_KEEP_RATIO = 0.90
    STRICT_MIN_H_FOR_SCORE = 0.20

    keep_idx: List[int] = [best_i]
    ...
    for i in cand_idx[1:]:
        ...
        if best_sc <= 0:
            # fallback: if best score is zero-ish, only keep by height
            keep = (h_ratio >= MIN_H_KEEP_RATIO)
        else:
            sc_ratio = sc / best_sc

            if best_h_ratio >= BIG_BEST_H:
                # mostly-single-panel pages: only keep additional if truly meaningful
                keep = (h_ratio >= MIN_H_KEEP_RATIO) or (sc_ratio >= STRICT_SCORE_KEEP_RATIO and h_ratio >= STRICT_MIN_H_FOR_SCORE)
            else:
                # normal pages: keep if tall enough OR close enough in score and not tiny
                keep = (h_ratio >= MIN_H_KEEP_RATIO) or (sc_ratio >= SCORE_KEEP_RATIO and h_ratio >= MIN_H_FOR_SCORE)

        if keep:
            keep_idx.append(i)

        if len(keep_idx) >= max_shots:
            break

...
Fixes / improvements:
- Eleven v3 requires stability ∈ {0.0, 0.5, 1.0}. We quantize automatically when model_id is v3.
- Keep bracket tags by default for v3 models; strip by default for non-v3 (same as before).
- Map your internal leading mood tags ([tense], [urgent], ...) into v3-friendly audio tags.
- Optional per-tag presets (also v3-safe).
- Actually send "speed" inside voice_settings.
- Safer clamping for style in [0..1].
"""
...
def _quantize_v3_stability(x: float) -> float:
    """
    Eleven v3 TTD requires stability to be one of:
      0.0 (Creative), 0.5 (Natural), 1.0 (Robust)
    We choose nearest bucket.
    """
    if x <= 0.25:
        return 0.0
    if x <= 0.75:
...
# Your script_expander emits: calm/tense/urgent/excited/awe/sad/whisper/angry
...
# Per-tag settings (v3-safe stabilities only)
_PRESET_BY_TAG: Dict[str, Dict[str, Any]] = {
    "calm": {"stability": 0.5, "style": 0.25, "speed": 1.08},
    "tense": {"stability": 0.5, "style": 0.45, "speed": 1.10},
    "urgent": {"stability": 0.0, "style": 0.55, "speed": 1.16},
    "excited": {"stability": 0.0, "style": 0.60, "speed": 1.14},
    "awe": {"stability": 0.5, "style": 0.45, "speed": 1.02},
    "sad": {"stability": 1.0, "style": 0.35, "speed": 1.00},
    "whisper": {"stability": 1.0, "style": 0.30, "speed": 1.02},
    "angry": {"stability": 0.0, "style": 0.60, "speed": 1.10},
}
...
def apply_v3_tag_mapping(text: str) -> Tuple[str, Optional[str]]:
    """
    If text starts with your internal tag [tense]/[urgent]/..., replace with v3-friendly audio tag.
    ...
    mapped = _TAG_MAP_V3.get(internal_tag)
    if not mapped:
...
def _coerce_voice_settings(vs: Dict[str, Any], *, v3: bool) -> Dict[str, Any]:
    """
    Coerce manifest/CLI voice_settings into a request-safe dict.
    - v3: quantize stability to {0.0, 0.5, 1.0}
    """
    out: Dict[str, Any] = {}
    ...
    if "style" in vs:
        out["style"] = _clamp01(float(vs["style"]))
...
def main() -> int:
    ...
    # Tag behavior:
    # - For v3 models: KEEP tags by default.
    # - For non-v3: STRIP tags by default (cosmetic).
    ...
    # Defaults (used when no tts_meta overrides are present)
    # NOTE: for v3 stability will be quantized to 0.0/0.5/1.0 automatically.
    ...
    # Decide stripping default
    # If v3: keep tags unless user explicitly wants stripping (by NOT passing --no-strip-tags?).
    # We'll interpret --no-strip-tags as "force keep tags".
    # If non-v3: strip unless user explicitly keeps tags.
    ...
    index: Dict[str, Any] = {
        ...
        "default_voice_settings": {
            "stability": (_quantize_v3_stability(float(args.stability)) if v3 else float(args.stability)),
            "similarity_boost": float(args.similarity_boost),
            "style": _clamp01(float(args.style)),
    ...
    for it in items:
        ...
        # Per-clip voice settings override (from manifest tts_meta) OR defaults
        ...
        # Optional: reinforce your internal tag via voice_settings presets (only when not overridden by tts_meta)
        if (not args.disable_tag_presets) and (not isinstance(vs, dict)) and internal_tag in _PRESET_BY_TAG:
            preset = _PRESET_BY_TAG[internal_tag]
            voice_settings["stability"] = (
                float(preset.get("stability", voice_settings.get("stability")))
                if not v3
                else _quantize_v3_stability(float(preset.get("stability", voice_settings.get("stability", 0.5))))
            )
            voice_settings["style"] = _clamp01(float(preset.get("style", voice_settings.get("style", args.style))))
